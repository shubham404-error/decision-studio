"""CapitalSense Decision Studio — a public, session-only Streamlit MVP."""
...
st.set_page_config(page_title="CapitalSense Decision Studio", page_icon="◈", layout="wide", initial_sidebar_state="expanded")
...
def init_state() -> None:
    defaults = {"ai_calls": 0, "holdings": [], "image_drafts": [], "portfolio_confirmed": False}
...
def home_page() -> None:
    st.markdown('<div class="cs-hero"><div class="cs-kicker">CapitalSense • Decision Studio</div><h1>Make the next financial decision clearer.</h1><p>Guided, deterministic tools for Indian investors. Start with the decision, then see the assumptions, trade-offs, and next safe check.</p></div>', unsafe_allow_html=True)
...
def portfolio_page() -> None:
    ...
    with st.expander("1. Add holdings", expanded=True):
        ...
        if st.button("Confirm holdings for analysis"):
            ...
            st.success("Holdings are confirmed for this session. Nothing was saved outside it.")
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption("Decision Studio · public MVP")
    ...
    pages[page]()
    st.divider()
    st.caption("CapitalSense Decision Studio is educational decision support. Verify tax rules, prices, instrument data, and suitability independently before acting.")
